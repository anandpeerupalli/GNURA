function [lenDoppOS, minDopp, DoppOS, sat_el1, sat_el2, mattime] = VESTA_DopplerCalcV10( ...
    tstart, passdurnsec, SHLatLong, GSLatLong,  fsband, fvhf, Doppname, demopass)
%
...
%     load VESTA_Doppler_variables_4Dec2018
    ...
    [sat_el1,sat_az1,pos1,range1,rangeRate1] = satElevation(mattime,6,GSLatLong);     % gateway station S-band
    [sat_el2,sat_az2,pos2,range2,rangeRate2] = satElevation(mattime,6,SHLatLong);     % ship station VHF band
...
if(sampspersec >= 0.01 && ~debugflg)
    ...
    savewf(DoppT/1e3, nam, savWav, savBin, savDec, savFloat, saveFolder, backoffdB, sampspersec, wavBitDepth, endian)
    nam         = 'Doppler3_total_norm';

    savewf(signal1, nam, savWav, savBin, savDec, savFloat, saveFolder, backoffdB, sampspersec, wavBitDepth, endian)
    nam         = 'Doppler3_total_Rnorm';

    savewf(signal1r, nam, savWav, savBin, savDec, savFloat, saveFolder, backoffdB, sampspersec, wavBitDepth, endian)
end
...
fs          = 24 * 9.6e3;           % approx. 3 kHz margin to min/max Doppler in Fs/4 units (to DC and Fs/2)
...
if(~debugflg)
    savewf(signal2, Doppname, savWav, savBin, savDec, savFloat, saveFolder, backoffdB, fs, wavBitDepth, endian)
end
